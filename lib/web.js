...
app.get('/download', function(req, res) {
  ...
  if (file === undefined) {
    res.status(400).send({error: 'missing required parameter "file".'}).end();
    return;
  }

  var justFileName = path.basename(file);
  // Strip leading '/'
  file = file.replace(/^\/+/, '');
  var spl = file.split('/');
  var base = spl[0];
  ...
  utils.errResWrap(fs.stat, file)
    .then(function checkIfDir(stat) {
      if (!stat.isFile()) {
        throw 'Path is not a reqular file.';
      }
      return [file, stat];
    })
    .then(function(fileAndStat) {
      var file = fileAndStat[0];
      var stat = fileAndStat[1];
